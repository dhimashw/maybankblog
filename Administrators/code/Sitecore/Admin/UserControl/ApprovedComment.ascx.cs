...
{
    public partial class ApprovedComment : System.Web.UI.UserControl
    {
        ...
        public void DisplayAllComment(bool setTab = false)
        {
            ...
            if (setTab)
            {
                Page.ClientScript.RegisterStartupScript(this.GetType(), "activeTab", $@"$(document).ready(function () {{ SetActiveTab('approved-comment') }});", true);
            }
        }
        ...
        {
            // @selectAll bit,
            // @selectDraft bit, approved-comment = 0
            // @keyword varchar(max),
            ...
            using (SqlConnection conn = new SqlConnection(new Config().ConectionString))
            {
                ...
                SqlParameter param1 = cmd.Parameters.AddWithValue("@selectDraft", 0);
